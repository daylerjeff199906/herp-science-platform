...
import { Individual, Sex, Activity, Museum, ForestType } from '@repo/shared-types';
...
import { createIndividual, updateIndividual, CreateIndividualData } from '@/services/individuals';
...
export function IndividualForm({
    ...
    useEffect(() => {
        if (initialData) {
            setFormData({
                ...
                activityId: initialData.activity?.id ? String(initialData.activity.id) : '',
                forestTypeId: initialData.forestType?.id ? String(initialData.forestType.id) : '',
                museumId: initialData.museum?.id ? String(initialData.museum.id) : '',
                speciesId: initialData.species?.id ? String(initialData.species.id) : '',
                occurrenceId: initialData.ocurrence?.id ? String(initialData.ocurrence.id) : '',
            });
        }
    }, [initialData]);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const submitData: CreateIndividualData = {
                ...
                forestTypeId: formData.forestTypeId ? Number(formData.forestTypeId) : undefined,
                museumId: Number(formData.museumId),
                speciesId: formData.speciesId ? Number(formData.speciesId) : 0,
                occurrenceId: formData.occurrenceId ? Number(formData.occurrenceId) : undefined,
                identifiers: [],
            };
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <Card>
                ...
                <CardContent className="space-y-4 pt-0">
                    <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                        ...
                        <div className="space-y-2">
                            <Label htmlFor="geneticBarcode" className="text-xs">Código Genético</Label>
                            <Input
                                id="geneticBarcode"
                    ...
                    <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
                        <div className="space-y-2">
                            <Label htmlFor="identDate" className="text-xs">Fecha de Identificación *</Label>
                            <Input
                                id="identDate"
                                type="date"
                                ...
                                className="h-9 text-xs"
                            />
                            {errors.identDate && <span className="text-xs text-red-500">{errors.identDate}</span>}
                        </div>

                        <div className="space-y-2">
                            <Label htmlFor="identTime" className="text-xs">Hora de Identificación *</Label>
                            <Input
                                id="identTime"
                                type="time"
                                ...
                                className="h-9 text-xs"
                            />
                            {errors.identTime && <span className="text-xs text-red-500">{errors.identTime}</span>}
                        </div>
                        ...
                        <div className="flex items-center space-x-2 pt-6">
                            <Checkbox
                                ...
                                onCheckedChange={(checked) => updateField('hasEggs', checked as boolean)}
                            />
                            <Label htmlFor="hasEggs" className="text-xs">¿Tiene huevos?</Label>
                        </div>
                    </div>
                </CardContent>
            </Card>
                    ...
                    <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                        <div className="space-y-2">
                            ...
                            >
                                ...
                                <SelectContent>
                                    {sexes.map((sex) => (
                                        <SelectItem key={sex.id} value={String(sex.id)} className="text-xs">
                                            {sex.name}
                                        </SelectItem>
                                    ))}
                                </SelectContent>
                            </Select>
                            {errors.sexId && <span className="text-xs text-red-500">{errors.sexId}</span>}
                        </div>
                            ...
                            >
                                ...
                                <SelectContent>
                                    {activities.map((activity) => (
                                        <SelectItem key={activity.id} value={String(activity.id)} className="text-xs">
                                            {activity.name}
                                        </SelectItem>
                                    ))}
                                </SelectContent>
                    ...
                    <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                        <div className="space-y-2">
                            ...
                            >
                                ...
                                <SelectContent>
                                    {museums.map((museum) => (
                                        <SelectItem key={museum.id} value={String(museum.id)} className="text-xs">
                                            {museum.name}
                                        </SelectItem>
                                    ))}
                                </SelectContent>
                            </Select>
                            {errors.museumId && <span className="text-xs text-red-500">{errors.museumId}</span>}
                        </div>
            ...
            <Card>
                ...
                <CardContent className="space-y-4 pt-0">
                    <div className="grid grid-cols-1 gap-4 md:grid-cols-4">
                        <div className="space-y-2">
                            <Label htmlFor="weight" className="text-xs">Peso (g)</Label>
                            <Input
                                id="weight"
                                type="number"
                                step="0.01"
                        ...
                        <div className="space-y-2">
                            <Label htmlFor="slaughteredWeight" className="text-xs">Peso Faenado (g)</Label>
                            <Input
                                id="slaughteredWeight"
                                type="number"
                                step="0.01"
                                value={formData.slaughteredWeight}
                                onChange={(e) => updateField('slaughteredWeight', e.target.value)}
                        ...
                        <div className="space-y-2">
                            <Label htmlFor="svl" className="text-xs">LC (mm)</Label>
                            <Input
                                id="svl"
                                type="number"
                                step="0.01"
                        ...
                        <div className="space-y-2">
                            <Label htmlFor="tailLength" className="text-xs">Longitud Cola (mm)</Label>
                            <Input
                                id="tailLength"
                                type="number"
                                step="0.01"
            ...
            <Card>
                <CardHeader className="py-3">
                    <CardTitle className="text-sm font-semibold">Información Adicional</CardTitle>
                </CardHeader>
                <CardContent className="space-y-4 pt-0">
                    <div className="space-y-2">
                        <Label htmlFor="depositCodeGenbank" className="text-xs">Código de Depósito GenBank</Label>
                        <Input
                            id="depositCodeGenbank"
                            value={formData.depositCodeGenbank}
                            onChange={(e) => updateField('depositCodeGenbank', e.target.value)}
